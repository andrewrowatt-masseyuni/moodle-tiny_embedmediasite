...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.handleAction=void 0,_modal=_interopRequireDefault(_modal),_templates=_interopRequireDefault(_templates);_exports.handleAction=async editor=>{displayDialogue(editor)};const displayDialogue=async editor=>{let page=1,presentations=await(0,_repository.getMyMediasitePresentations)(page).catch((error=>(0,_notification.exception)(error)));const modal=await _modal.default.create({templateContext:presentations,large:!0,removeOnClose:!0});await modal.show();document.getElementById("tiny_embedmediasite_content-container").addEventListener("click",(async event=>{const target=event.target;if(target&&target.classList.contains("tiny-embedmediasite-insert-button")){var _container$querySelec,_container$querySelec2;event.preventDefault();const mode=target.dataset.mode,container=target.closest(".presentation"),source=container.dataset.source,id=container.dataset.id,title=container.dataset.title,description=(null===(_container$querySelec=container.querySelectorAll('input[name="'.concat(id,'description"][type="checkbox"]'))[0])||void 0===_container$querySelec?void 0:_container$querySelec.checked)?null===(_container$querySelec2=container.querySelectorAll(".description p")[0])||void 0===_container$querySelec2?void 0:_container$querySelec2.innerText:"";let templateName;templateName="linkonly"===mode?"tiny_embedmediasite/_linkonly":"tiny_embedmediasite/_embedlink";const{html:html}=await _templates.default.renderForPromise(templateName,{source:source,title:title,description:description});editor.insertContent(html),modal.destroy()}}));const loadingIndicator=document.getElementById("tiny_embedmediasite_loading"),noMoreContentIndicator=document.getElementById("tiny_embedmediasite_no_more_content");const observer=new IntersectionObserver((async entries=>{entries[0].isIntersecting&&(observer.unobserve(loadingIndicator),page++,await async function(pageNumber){var _presentations$list;const presentations=await(0,_repository.getMyMediasitePresentations)(pageNumber).catch((error=>(0,_notification.exception)(error)));if(null===(_presentations$list=presentations.list)||void 0===_presentations$list||!_presentations$list.length)return 0;const{html:html,js:js}=await _templates.default.renderForPromise("tiny_embedmediasite/_presentations",presentations);return _templates.default.appendNodeContents("#tiny_embedmediasite_content-container",html,js),presentations.list.length}(page)?observer.observe(loadingIndicator):(loadingIndicator.style.display="none",noMoreContentIndicator.style.display="block"))}),{root:null,threshold:1,rootMargin:"0px"});observer.observe(loadingIndicator)}}));
